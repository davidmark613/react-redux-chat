...
class Messages extends Component {
    ...
    displayChannelName = channel => {
        return channel ? `${this.state.privateChannel
            ? '@' : '#'}${channel.name}`
            : '';
    };
    ...
    displayTypingUsers = users => (
        users.length > 0 && users.map(user => (
            <div style={{display: 'flex', alignItems: 'center', marginBottom: '0.2em'}} key={user.id}>
                <span className='userTyping'>{user.name} is typing</span> <Typing/>
            </div>
        ))
    )
    ...
    render() {
        const { messagesRef, messages, channel, user,
                ...
                searchResults, searchLoading, privateChannel, isChannelStarred,
        ...
        return (
            <React.Fragment>
                ...
                <Segment>
                    <Comment.Group className={progressBar
                        ? 'messagesProgress'
                        : 'messages'
                    }>
